    ...
    public ResponseEntity<List<Appointment>> getAppointments(
        ...
        if (date != null && !date.isBlank() && doctorId != null && !doctorId.isBlank()) {
            ...
            if (doctorOptional.isEmpty()) {
                return ResponseEntity.badRequest().build(); // 醫師不存在
            }
            appointments = appointmentRepo.findByApptDateAndDoctor(apptDate, doctorOptional.get());
            ...
            appointments = appointmentRepo.findByDoctor(doctorOptional.get());
        } else {
            // 兩者皆未提供，回傳所有掛號
